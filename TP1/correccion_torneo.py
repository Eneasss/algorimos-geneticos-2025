"""
ALGORITMO GENÉTICO CON SELECCIÓN POR TORNEO

Este programa implementa un algoritmo genético simple para maximizar la función f(x) = (x/(2^30-1))^2.
Utiliza una población de 10 individuos, cada uno representado por un cromosoma de 30 bits.
El método de selección implementado es el TORNEO, donde se seleccionan aleatoriamente 4 individuos
y se elige al que tiene mayor fitness. También implementa operadores de cruce (crossover) de un punto
y mutación de un bit con probabilidades del 75% y 5% respectivamente.

El algoritmo ejecuta 20 ciclos (generaciones) y muestra el estado de la población en cada generación,
guardando también los resultados en un archivo CSV.
"""
...
def Torneo(seleccion, fitness):
    """
    Implementa el método de selección por torneo.
    Para cada posición, selecciona aleatoriamente 4 individuos y elige al de mayor fitness.
    ...
    """
    for j in range(10):
        torneo = []  # Almacena los valores de fitness de los participantes
        postorneo = []  # Almacena las posiciones de los participantes
        ...
        # Encuentra al individuo con mayor fitness en el torneo
        ganador = torneo[0]
        ...
        # El ganador del torneo es seleccionado
...
def CrossOver(poblacion, pob_siguiente, seleccion):
    ...
    for c in range(0, 10, 2):  # Procesa los individuos de dos en dos
        pad1 = seleccion[c]
        pad2 = seleccion[c + 1]
...
def main():
    ...
    ciclos = 20             # Número de generaciones a ejecutar
