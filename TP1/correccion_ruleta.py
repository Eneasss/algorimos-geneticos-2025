...
El método de selección implementado es la RULETA, donde la probabilidad de selección es proporcional
al valor de fitness de cada individuo. También implementa operadores de cruce (crossover) de un punto
...
def Ruleta(seleccion, fitness):
    """
    Implementa el método de selección por ruleta.
    Los individuos con mayor fitness tienen más probabilidad de ser seleccionados.
    ...
    """
    fit = [0] * 10
    ruleta = [0] * 120  # Arreglo que representa la ruleta
    rul = 0
    max_idx = 0
    total_fit = 0

    # Convierte los valores de fitness a enteros (porcentajes)
    for c in range(10):
    ...
    # Selecciona 10 individuos girando la ruleta
    for c in range(10):
        i = random.randint(0, 99)  # Genera un número aleatorio entre 0 y 99
